...
# from .utils import remove, replace_tag_type, replace_informative,retain_informative,retain_informative_i2b2
from .utils import *

class TextDeid(object):

    def __init__(self, notation, span_constraint,output_AI_cup_prediction_file):
        self._span_constraint = span_constraint
        self.output_AI_cup_prediction_file = output_AI_cup_prediction_file
    ...
    def run_deid(
        ...
        if os.path.isfile(self.output_AI_cup_prediction_file):
            os.remove(self.output_AI_cup_prediction_file)
        note_map = dict()
        ...
        for line in open(predictions_file, 'r'):
            ...
            yield TextDeid.__get_deid_text(
                deid_note=deid_note,
                output_AI_cup_prediction_file=self.output_AI_cup_prediction_file,
    ...
    def __get_deid_text(
        ...
        with open(output_AI_cup_prediction_file,"a") as predict_file:
            for entity in entity_positions:
                # if deid_note['meta']['note_id']=="1004":
                #     print("error")
                #     # continue
                #     print(deid_note['meta']['note_id']+"\t"+entity[1]+"\t"+str(entity[0][0])+"\t"+str(entity[0][1])+"\t"+note_text[entity[0][0]:entity[0][1]])
                # if entity[1]!="":
                span = note_text[entity[0][0]:entity[0][1]]
                if "\n" in note_text[entity[0][0]:entity[0][1]]:
                    span = span.split("\n")[0]
                    end_posi = entity[0][1]-2
                    if " " in span:
                        span = span.strip()
                        end_posi-=1
                    if str(entity[0][0]) != str(end_posi):
                        predict_file.write(deid_note['meta']['note_id']+"\t"+entity[1]+"\t"+str(entity[0][0])+"\t"+str(end_posi)+"\t"+span+"\n")
                else:
                    if span.strip() not in [" ",""]:
                        predict_file.write(deid_note['meta']['note_id']+"\t"+entity[1]+"\t"+str(entity[0][0])+"\t"+str(entity[0][1])+"\t"+span+"\n")
        ...
        for positions, tag in entity_positions:
            ...
            if tag == 'AGE' and keep_age:
                ...
                if regex.search(age_pattern, span_text, flags=regex.IGNORECASE):
                    ...
                    deid_tag = deid_tag
            if (deid_strategy == 'replace_informative' or deid_strategy == 'ai_cup') and not age_unchanged:
    ...
    def __get_tag_mapping(deid_strategy):
        ...
        elif deid_strategy == 'ai_cup':
            return retain_informative()
        elif deid_strategy == 'ai_cup_i2b2':
            return retain_informative_i2b2()
    @staticmethod

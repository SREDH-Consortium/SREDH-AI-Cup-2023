...
class SequenceTagger(object):
    ...
    def evaluate(self, output_predictions_file: Optional[str] = None):
        if self._eval_dataset is not None and self._trainer is not None:
            # Evaluation
            self._logger.info("*** Evaluate ***")
            # unique_note_ids = set()
            # for note_sent_info in self._eval_dataset['note_sent_info']:
            ...
            #     sequences=self._eval_dataset['current_sent_info']
            # )
            # predictions, labels, metrics = self._trainer.evaluate()

...
class DatasetTokenizer(object):
    ...
    def tokenize_and_align_labels(self, dataset: Dataset) -> Dataset:
        ...
        # Run the tokenizer - subword tokenization (origin)
        ...
        # new_attention_mask = []
        # cnt = -1
        # for tok_sentence in dataset[self._token_column]:
        #     tmp_tokenized_inputs = self._tokenizer.tokenize(
        #         tok_sentence,
        #         padding=self._padding,
        #         truncation=self._truncation,
        #         max_length=self._max_length,
        ...
        #     tmp_word_ids = []
        #     tmp_attention_mask = []
        #     for tok in tmp_tokenized_inputs:
        #         if "Ġ" not in tok:
        #             tmp_tok = self._tokenizer.encode(self._tokenizer.convert_tokens_to_string(" "+tok),add_special_tokens=False)
        #             remove_connect.extend(tmp_tok)
        #             tmp_word_ids.extend([cnt]*len(tmp_tok))
        #             tmp_attention_mask.extend([1]*len(tmp_tok))
        #         else:
        #             cnt+=1
        #             tok = tok.replace("Ġ"," ")
        #             tmp_tok = self._tokenizer.encode(tok,add_special_tokens=False)
        #             remove_connect.extend(tmp_tok)
        #             tmp_word_ids.extend([cnt]*len(tmp_tok))
        #             tmp_attention_mask.extend([1]*len(tmp_tok))
        #     new_tokenized_inputs.append([self._tokenizer.bos_token_id]+remove_connect+[self._tokenizer.eos_token_id])
        #     new_word_ids.append([None]+tmp_word_ids+[None])
        #     new_attention_mask.append([1]+tmp_attention_mask+[1])
        #     cnt=-1

        # tokenized_inputs['input_ids'] = new_tokenized_inputs
        # tokenized_inputs['attention_mask'] = new_attention_mask
